...
import src.java.audio.BackgroundMusic;
import src.java.core.entities.Alien;
...
public class Board extends JPanel implements Runnable, MouseListener {
    ...
    int alienTolerance = 7; //So weit vom src.java.core.entities.Alien darf ein Schuss weg sein, um dennoch zu zählen → Hiermit kann man das Spiel einfacher machen
    ...
    /**
     * Im Konstruktor von src.java.ui.Board wird das Spiel initialisiert, d.h. zum Beispiel die Hintergrundmusik gestartet und ihre Llutstärke geregelt, der Mouse- und KeyListener hinzugefügt und die Animationsschleife gestartet.
     * @param starter
     * @param sandBox
    ...
    public Board(Starter starter, boolean sandBox, float setBackgroundVolume, float setSoundVolume, Menu menu) {
        //Verweist auf src.java.app.Starter
    ...
    public void spawnAliens() {
        ...
        int ax = 15;
        int ay = 95;
        //Schleife geht durch jedes src.java.core.entities.Alien auf dem Bildschirm durch und berechnet die neue Position
    ...
    public void paint(Graphics g) {
        ...
        for (int i = 0; i < Aliens.size(); i++) {
            ...
            //Zeichnet das src.java.core.entities.Alien an der Stelle i in der ArrayListe
    ...
    public void updateVars(Graphics g) {
        ...
        g.setFont(small);
        g.setColor(Color.white);
        //Gibt für jedes src.java.core.items.Item die eingesammelte Anzahl an (zeichnet diese)
    ...
    public void drawItems(Graphics g) {
        ...
        if (Items.size() != 0) {
            ...
            for (int i = 0; i < Items.size(); i++) {
                //Überprüft welcher src.java.core.items.Item-Typ gegeben ist und zeichnet diesen
                if (Items.get(i).type == "src.java.core.items.Heart") {
                    g.drawImage(HeartImg, Items.get(i).x, Items.get(i).y, this);
                }
                if (Items.get(i).type == "src.java.core.items.ShotSpeedUp") {
                    g.drawImage(ShotSpeedUpImage, Items.get(i).x, Items.get(i).y, this);
                }
                if (Items.get(i).type == "src.java.core.items.MoveSpeedUp") {
                ...
                //Setzt das src.java.core.items.Item abhängig von seinem Speed weiter nach unten
                Items.get(i).y += Items.get(i).speed;
                //Überprüft, ob das src.java.core.items.Item vom Spieler eingesammelt wurde und verändert die Werte dementsprechend
                if (Items.get(i).x >= p.x - playerItemTolerance && Items.get(i).x <= p.x + playerWidth + playerItemTolerance && Items.get(i).x + shotWidth >= p.x - playerItemTolerance && Items.get(i).x + shotWidth <= p.x + playerWidth + playerItemTolerance && Items.get(i).y >= p.y && Items.get(i).y <= p.y + playerWidth) {
                    if (Items.get(i).type == "src.java.core.items.Heart" && ((!sandBox && HP < MAX_HP) || (sandBox && HP < 10))){
                        HP++;
                    }
                    if (Items.get(i).type == "src.java.core.items.ShotSpeedUp" && ((!sandBox && shotCooldown > 250) || (sandBox && shotCooldown >= 50))) {
                        shotCooldown -= 50;
                        ShotSpeedUpAnzahl++;
                    }
                    if (Items.get(i).type == "src.java.core.items.MoveSpeedUp" && p.speed < pMaxSpeed) {
                        p.speed++;
                        MoveSpeedUpAnzahl++;
                    }
    ...
    public void drawExplosions(Graphics g) {
        ...
        for (int i = 0; i < Explosions.size(); i++) {
            ...
            //Überprüft für jede src.java.ui.Explosion welches frame der Animation gezeichnet werden soll und zeichnet dieses
            ...
            if (frame <= 7 && System.currentTimeMillis() - Explosions.get(i).lastFrameTime >= 65) {
                ...
                Explosions.remove(i); //Wenn alle frames abgespielt wurden, soll die src.java.ui.Explosion wieder entfernt werden
        }
    }
    ...
    public String getGameOverSoundEffect() {
        ...
        if (r <= 0.17) return "audioTracks/GameOver0.wav";
        else if (r <= 0.33) return "audioTracks/GameOver1.wav";
        else if (r <= 0.5) return "audioTracks/GameOver2.wav";
        else if (r <= 0.67) return "audioTracks/GameOver3.wav";
        else if (r <= 0.83) return "audioTracks/GameOver4.wav";
        else return "audioTracks/GameOver5.wav";
    }
    ...
    public void moveAliens() {
        ...
        if (System.currentTimeMillis() - aLastMove >= aMoveCooldown) {
            ...
            for (int i = 0; i < Aliens.size(); i++) {
                ...
                if (Aliens.get(i).moveLeft) {
                    Aliens.get(i).x -= Aliens.get(i).speed;
                }
                if (Aliens.get(i).moveRight) {
                    Aliens.get(i).x += Aliens.get(i).speed;
                }
            }
        ...
        //Wenn ein src.java.core.entities.Alien einen gewissen Abstand von den Rändern des Spielfensters erreicht hat,
        ...
        if ((System.currentTimeMillis() - aLastShotTime) >= aShotCooldown && Aliens.size() != 0) {
            //Wählt ein zufälliges src.java.core.entities.Alien aus
            int r = (int) Math.round(Math.random() * (Aliens.size() - 1));
            //Erzeugt einen Schuss in diesem src.java.core.entities.Alien und setzt die "hostile" Variable für diesen auf true, damit er nur den Spieler treffen kann und sich nach unten bewegt
            ...
            Soundeffects.music("audioTracks/Shot.wav", setSoundVolume);
        }
    }
     ...
     * Wenn die Spielerleben auf Null sinken, wird eine src.java.ui.Explosion inklusive zugehörigem Soundeffect erstellt.
     * @param g
     */
        ...
        for (int i = 0; i < AShots.size(); i++) {
            ...
            if (!AShots.isEmpty()) {
                if (AShots.get(i).x >= p.x - playerTolerance && AShots.get(i).x <= p.x + playerWidth + playerTolerance && AShots.get(i).x + shotWidth >= p.x - playerTolerance && AShots.get(i).x + shotWidth <= p.x + playerWidth + playerTolerance && AShots.get(i).y >= p.y && AShots.get(i).y <= p.y + playerWidth) {
                    ...
                    if(hearts) {
                        heartsLost++;
                        Soundeffects.music("audioTracks/player_gethit.wav", setSoundVolume);
                        if (HP > 0) {
                            HP--;
                            if (HP == 0) {
                                ...
                                Soundeffects.gameOver("audioTracks/explosionSound.wav", getGameOverSoundEffect(), setSoundVolume);
                                drawExplosions(g);
                            }
                        }
                    }
    ...
    /**
     * Wird in paint() aufgerufen und bewegt die Schüsse des Spielers und überprüft ob ein src.java.core.entities.Alien getroffen wurde.
     * Wird ein src.java.core.entities.Alien getroffen, wird an dessen Stelle eine src.java.ui.Explosion mit zugehörigem Soundeffekt erzeugt.
     */
    public void movePShots() {
        ...
        for (int i = 0; i < PShots.size(); i++) {
            ...
            //Überprüft für jedes src.java.core.entities.Alien, ob es von einem Schuss getroffen wurde
            for (int j = 0; j < Aliens.size(); j++) {
                if (!PShots.isEmpty() && Shot && i < PShots.size()) {
                    if (PShots.get(i).x >= Aliens.get(j).x - alienTolerance && PShots.get(i).x <= Aliens.get(j).x + alienWidth + alienTolerance && PShots.get(i).x + shotWidth >= Aliens.get(j).x - alienTolerance && PShots.get(i).x + shotWidth <= Aliens.get(j).x + alienWidth + alienTolerance && PShots.get(i).y >= Aliens.get(j).y && PShots.get(i).y <= Aliens.get(j).y + alienWidth) {
                        //Bei jedem erledigten src.java.core.entities.Alien gibt es eine Chance, dass ein PowerUp erzeugt wird, dies ist in der Methode dropChance() erklärt
                        if(powerUps){
                            dropChance(Aliens.get(j));
                        }
                        //Wird ein src.java.core.entities.Alien getroffen, wird sowohl das src.java.core.entities.Alien, wie auch der entsprechende Schuss entfernt und die Anzahl der Kills, sowie der Score erhöht
                        PShots.remove(i);
                        //Eine src.java.ui.Explosion wird bei den src.java.core.entities.Alien-Koordinaten erzeugt
                        Explosions.add(new Explosion(Aliens.get(j).x, Aliens.get(j).y, 1));
                        Soundeffects.music("audioTracks/explosionSound.wav", setSoundVolume);
                        Aliens.remove(j);
                        kills++;
                        score += alienworth;
    ...
    public void mouseClicked(MouseEvent e) {
        ...
        else if(sandBox && mouseX >= 350 && mouseX <= 380 && mouseY >= 60 && mouseY <= 90 && MultiShotChance < 0.9){ //erlaubt im Sandbox-Modus die Anzahl eingesammelter src.java.core.items.MultiShot-Powerups zu erhöhen (siehe oberes) und zu verringern (siehe unteres)
            MultiShotChance += 0.1;
            MultiShotUpAnzahl++;}
    ...
    private class TAdapter extends KeyAdapter {
        //src.java.ui.Board wird später gebraucht, um es in src.java.app.Starter über einen hier eingelesenen Knopfdruck zu entfernen
        Board board;
        ...
        public void keyPressed(KeyEvent e) {
            ...
            if (key == 49 && !ingame) {
                ...
                //Startet das Spiel neu (fügt wieder ein neues src.java.ui.Board hinzu)
